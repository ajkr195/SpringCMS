...
//	List<AppUser> findByUserdatecreated(Date userdatecreated);
	...
	@Query("SELECT u FROM AppUser u WHERE u.creationDate < ?1")
	List<AppUser> findByCreationdate (Date creationDate);
	
	@Query("SELECT u FROM AppUser u WHERE u.username = ?1")
	AppUser getAppuserIdByUsername(String username);
	
	@Query("SELECT u FROM AppUser u WHERE u.userconfirmationtoken = ?1")
